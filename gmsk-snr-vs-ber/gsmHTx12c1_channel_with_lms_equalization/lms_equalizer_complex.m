function [y, w_final, mse_curve] = lms_equalizer_complex(rx, s_ref, trainSampleIdx, mu, eq_len, decisionDelay, verbose)
...
%   decodedBits    = viterbi_soft_decoder(payloadMetrics, TB_LEN);
    ...
    % Convergence check: compare robust head/tail statistics of the MSE
    % curve. Median is less sensitive than mean to occasional error spikes.
    q    = max(1, floor(updateCount / 4));
    head = mse_curve(1:q);
    tail = mse_curve(end-q+1:end);
    m1   = mean(head);
    m2   = mean(tail);
    md1  = median(head);
    md2  = median(tail);
    ...
    %              '  MSE: initial=%.4f  final=%.4f | mean %.4f -> %.4f | median %.4f -> %.4f  %s\n' ...
    %              '  Tap power: %.4f\n\n'], ...
    %         updateCount, eq_len, mu, decisionDelay, ...
    %         mse_curve(1), mse_curve(end), m1, m2, md1, md2, status, ...
