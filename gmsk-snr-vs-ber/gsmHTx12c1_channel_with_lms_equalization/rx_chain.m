...
assert(waveform.burstBlockLen == ...
       ...
       'Waveform burstBlockLen mismatch â€” check Waveform.init()');
...
%% =========================================================================
%% SECTION 2: Coarse demodulation â€” preamble detection
%% =========================================================================
...
for d = 1:length(detections)
    ...
    if burstEndSample > length(rx_full)
        warning('Burst %d truncated by end of recording â€” skipping', d);
        continue;
    end
    ...
    if rxAmp < 1e-9
        warning('Burst %d: rx_burst amplitude near zero â€” likely empty burst, skipping', d);
        continue;
    end
...
%   [y_eq, w_final, mse_curve] = lms_equalizer_complex(rx_norm, s_ref, trainSampleIdx, mu, eq_len);
    ...
    if length(symMetrics) < waveform.burstBlockLen
        warning('Burst %d: symMetrics length %d < burstBlockLen %d â€” skipping', ...
