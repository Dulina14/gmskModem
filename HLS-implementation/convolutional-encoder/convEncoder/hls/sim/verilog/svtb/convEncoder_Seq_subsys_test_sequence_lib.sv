...
//
//==============================================================
`ifndef CONVENCODER_SEQ_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define CONVENCODER_SEQ_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
    `define AUTOTB_TVIN_s_axis_s_axis_TDATA  "../tv/cdatafile/c.convEncoder_Seq.autotvin_s_axis_V_data_V.dat" 
    `define AUTOTB_TVIN_s_axis_s_axis_TKEEP  "../tv/cdatafile/c.convEncoder_Seq.autotvin_s_axis_V_keep_V.dat" 
    `define AUTOTB_TVIN_s_axis_s_axis_TSTRB  "../tv/cdatafile/c.convEncoder_Seq.autotvin_s_axis_V_strb_V.dat" 
    `define AUTOTB_TVIN_s_axis_s_axis_TLAST  "../tv/cdatafile/c.convEncoder_Seq.autotvin_s_axis_V_last_V.dat" 
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class convEncoder_Seq_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "convEncoder_Seq_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(convEncoder_Seq_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(convEncoder_Seq_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            if (!uvm_config_db#(convEncoder_Seq_reference_model)::get(p_sequencer,"", "refm", refm))
            ...
            fork
                begin
                    fork
                        begin
                            ...
                            if( refm.convEncoder_Seq_cfg.port_s_axis_cfg.prt_type == AP_VLD ) wait(refm.misc_if.tb2dut_ap_start === 1'b1);
                        ...
                        begin
                            int delay;
                            for(int j=0; j<147; j=j+refm.ap_done_cnt) begin
                                @refm.dut2tb_ap_done;
                ...
                begin
                    for(int j=0; j<147; j=j+refm.ap_done_cnt) @refm.ap_done_for_nexttrans;
